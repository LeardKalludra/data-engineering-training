...
SELECT customer_name, item, price FROM food_orders;
...
SELECT * FROM food_orders 
WHERE status = 'completed' AND price > 10.00;
...
SELECT order_id, item, (quantity * price) AS total_order_cost 
FROM food_orders;

SELECT customer_name, SUM(quantity) AS total_items_ordered, SUM(quantity * price) AS total_amount_spent
FROM food_orders
WHERE status = 'completed'
GROUP BY customer_name
ORDER BY total_amount_spent DESC;

SELECT customer_name, item AS product, (quantity * price) AS total_amount, status
FROM food_orders
WHERE status = 'completed';